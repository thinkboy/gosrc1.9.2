...
type g struct {
	...
	sched          gobuf          // G的执行环境, 当G中断时会保存当前的PC和SP等值到这里, 恢复运行时会使用这里的值
...
type p struct {
	...
	runqhead uint32
	runqtail uint32
	runq     [256]guintptr // 存储将要运行的G队列，这是个典型的环形队列
	...
	gcMarkWorkerMode gcMarkWorkerMode // 当前P下的标记工作模式
...
type schedt struct {
	...
	npidle     uint32
	nmspinning uint32 // 代表正在创建M的个数 // See "Worker thread parking/unparking" comment in proc.go.
...
var (
	...
	gomaxprocs  int32
	ncpu        int32 // CPU数量，由初始化函数osinit()执行时得到
